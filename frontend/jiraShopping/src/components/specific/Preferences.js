...
//custom component
import MiniCategories from "../common/CategoriesMini";
...
const ProductsListWithFilters_ = React.memo(
  ({ onEndReached, isLoading, hasMore, products }) => {
    useEffect(() => {
      console.log("ok");
...
const Categories_ = React.memo(() => {
  return <Categories page="category" goBackTo="SearchResults" route={{}} />;
});
...
const Preferences = (props) => {
  ...
  const emptyMessage =
    "Pas de produits a afficher pour le moment. Nous vous preparons pour une mise a jour spectaculaire.";
  const emptyIcon = {
    type: "font-awesome-5",
    name: "box-open",
    size: 100,
...
*/
  ...
  const renderTabBar = (props) => (
    <TabBar
      ...
      indicatorStyle={{
        backgroundColor:
          index === 0 || index == 1 ? appColors.secondaryColor1 : false,
      }}
      style={{
        height: 0,
        backgroundColor: appColors.secondaryColor3,
        borderColor: appColors.white,
      }} // Changez cette couleur pour la barre d'onglets
        ...
        return (
          <Text
            style={{
              height: 0,
              fontSize: 16,
              color: focused ? appColors.secondaryColor1 : appColors.gray,
...

{
  /* <View
        style={[
          preferencesStyles.topAnimationBox,
          {
            backgroundColor: show3
              ? appColors.secondaryColor1
              : preferencesStyles.topAnimationBox.backgroundColor,
          },
        ]}
      >
        {show1 && (
          <LeftToRightViewBox
            ...
            duration={1000}
            from={-screenWidth}
            to={(screenWidth - text1Width) / 2}
            styles={{ position: "absolute" }}
          >
            <View
              onLayout={onText1Layout}
              style={[preferencesStyles.topAnimation]}
            >
              <Text
                style={[customText.text, preferencesStyles.animatedText, {}]}
              >
                Achats facile + négociation de prix...
              </Text>
            </View>
          </LeftToRightViewBox>
        )}
        ...
        {show2 && (
          <RightToLeftViewBox
            ...
            duration={1000}
            from={screenWidth}
            to={(screenWidth - text2Width) / 2}
            styles={{ position: "absolute" }}
          >
            <View
              onLayout={onText2Layout}
              style={[preferencesStyles.topAnimation]}
            >
              <Text
                style={[customText.text, preferencesStyles.animatedText, {}]}
              >
                Payment sécurisé et en un clic!
              </Text>
            </View>
          </RightToLeftViewBox>
        )}
        ...
        >
          <View onLayout={onText3Layout} style={[preferencesStyles.winkelBox]}>
            <Text style={[customText.text, preferencesStyles.winkelText, {}]}>
              Winkel
            </Text>
          </View>
        </LeftToRightViewBox>
