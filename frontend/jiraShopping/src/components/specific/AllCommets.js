...
const AllCommets = (props) => {
  ...
  const addComment = async (item) => {
    ...
    if (containsEmail(inputValue) || checkNumber == false) {
      Alert.alert(
        "Erreur!!!",
        "Votre commentaire viole les regles de la commnauté. Evitez de partager les contacts : numéro de téléphone, email, profil reseau sauciaux.\
    ...
    try {
      ...
      const response = await fetch(
        `${server}/api/datas/comments/add/${isResponseTo}`,
        {
          method: "POST",
      ...
      if (!isResponseTo) {
        ...
        if (user._id != userToResponse?._id) {
          await sendNotifications({
            user: userToResponse?._id,
            source: "app",
            model: "PRODUCTS",
            type: "ON_RESPONSE_COMMENT",
            datas: item._id,
          });
        }
      }
    ...
    <View style={{ flex: 1 }}>
      {reshapedComments.length <= 0 ? (
        <View style={[allCommetsStyles.container, { paddingBottom: 100 }]}>
          <EmptyList
            ...
            text="Ce produit n'a pas encore reçu de commentaires. Soyez le premier a commenter pour garantir votre achat."
          />
        </View>
      ) : (
        <View style={[allCommetsStyles.container, {}]}>
          <Comments
            ...
            user={user}
            setUserToResponse={setUserToResponse}
            flatListRef={flatListRef}
            inputRef={inputRef}
            setters={{
              inputValue: inputValue,
              setInputValue: setInputValue,
              setIsResponseTo: setIsResponseTo,
            }}
            all={true}
      ...
      <View style={[allCommetsStyles.inputContainer]}>
        {!!userToResponse && (
          <View>
            <View
              style={[{ height: 20, backgroundColor: appColors.white }]}
            ></View>
            <View style={[allCommetsStyles.isResponseTo]}>
              <Pressable
                View
                style={[allCommetsStyles.isResponseToClose]}
                onPress={() => {
                  setUserToResponse(null);
                  setIsResponseTo(null);
                }}
              ...
              <View>
                ...
                <View View style={[allCommetsStyles.isResponseToTextContainer]}>
                  <Text
                    style={[
                      customText.text,
                      { color: appColors.secondaryColor3, fontWeight: "bold" },
                    ]}
                  >
                    Vous répondez a - {userToResponse.username}
                  </Text>
                </View>
              </View>
            </View>
        ...
        <Input
          ...
          rightIcon={
            isLoading ? (
              ...
              <Pressable
                onPress={() => {
                  addComment(route.params.product);
                }}
      ...
      {isLoading && <ActivityIndicator color={appColors.secondaryColor1} />}
    </View>
  );
};
