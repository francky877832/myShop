...
const ProductDetails = (props) => {
  ...
  const modifiedProducts = useSelector(
    (state) => state.favourites.modifiedProducts
  );
  const modifiedProduct = modifiedProducts.filter(
    (product) => product?._id === route.params.productDetails._id
  );
  ...
  const [description, setDescription] = useState(
    truncateText(data.description, numChars)
  );
  ...
  const isFavourite = useSelector((state) =>
    isProductFavourite(state, data._id)
  );
  ...
  const [isBasketPresent, setIsBasketPresent] = useState(
    useSelector((state) => isProductBasket(state, data._id))
  );
  ...
  const _handleLikePressed = useCallback(
    (product) => {
      ...
      setLikeIcon((prevLike) => {
        ...
        newLike
          ? setLikeAdders(
              !likeAdders.some((item) => item._id === user._id)
                ? [user, ...likeAdders]
                : likeAdders
            )
          : setLikeAdders(likeAdders.filter((item) => item._id != user._id));
    ...
    <View style={productDetailsStyles.container}>
      {
        <>
          <View style={productDetailsStyles.buttonContainer}>
            ...
            <View style={productDetailsStyles.buttonContainerLeft}>
              <ShareButton
                ...
                size={24}
                styles={productDetailsStyles.shareButton}
                link={`${server}/product/${data._id}`}
              />
              ...
              <LikeButton
                ...
                synchro={true}
                item={data}
                styles={{ color: appColors.clearBlack }}
                isCard={false}
              />
            </View>
          </View>
          ...
          <Animated.View
            style={[
              productDetailsStyles.carousselIamges,
              { height: height.current },
            ]}
          >
            <CarouselImage
              style={productDetailsStyles.carousselIamge}
              images={data.images}
              product={data}
              styles={{}}
            />
      ...
      <ScrollView
        ...
        style={[productDetailsStyles.scrollView, { zIndex: 9999 }]}
        ref={scrollViewRef}
        horizontal={false}
        nestedScrollEnabled={true}
      >
        <View
          contentContainerStyle={[productDetailsStyles.getBackPosition, {}]}
        >
          {/*
          ...
          <View
            style={[
              productDetailsStyles.underCaroussel,
          ...
          >
            <View style={[productDetailsStyles.infosBox]}>
              <View style={productDetailsStyles.since}>
                <View style={{ flexDirection: "row" }}>
                  ...
                  >
                    Mis à jour il y'a {sinceDate(data.updatedAt).join(" ")}
                  </Text>
                </View>
                <View
                ...
                >
                  <LikeButton
                    ...
                    synchro={true}
                    item={data}
                    styles={{ color: appColors.black, fontWeight: "normal" }}
                    isCard={false}
                  />
                </View>
              </View>
              ...
              <View style={[productDetailsStyles.name]}>
                ...
                >
                  @{capitalizeFirstLetter(data.seller.username)}
                </Text>
                <View
                  style={[
                    {
              ...
              <View style={[productDetailsStyles.details]}>
                ...
                >
                  <BadgeIcon
                    ...
                    isCard={false}
                  />
                  <Text style={[customText.text, { paddingLeft: 5 }]}>
                ...
                >
                  ...
                  <Text style={[customText.text, { paddingLeft: 5 }]}>
                    {capitalizeFirstLetter(data.condition)}
                  </Text>
                </View>
                {data.color == "multicolor" ? (
                  ...
                  >
                    ...
                    />
                    <View style={{ width: 5 }}></View>
                    <Text style={[customText.text]}>
                      {capitalizeFirstLetter(data.color).substring(0, 10)}
                    </Text>
                  </View>
                ) : (
                  <View
              ...
              <View style={[productDetailsStyles.description]}>
                ...
                >
                  <Text style={[customText.text, { fontWeight: "bold" }]}>
                    Description
                  </Text>
                  <Text
                    style={[
                      customText.text,
                      { fontWeight: "bold", color: appColors.gray },
                    ]}
                  >
                    {data.garanti > 0
                      ? `${data.garanti} Mois . Garanti`
                      : false}{" "}
                  </Text>
                </View>
                ...
                <View style={[productDetailsStyles.descriptionBox]}>
                  ...
                  {description[1] == 0 && !description[2] ? (
                    ...
                    <Pressable
                      onPress={() => {
                        setDescription(
                          truncateText(data.description, numChars, true)
                        );
                      }}
                    >
                      <Text
            ...
            <View style={[productDetailsStyles.commentsContainer]}>
              <Pressable
                style={[productDetailsStyles.sellerBrand]}
                onPress={() => {
                  handleSellerBrandPressed(data);
                }}
              >
                <SellerBrand
                  pub={true}
                  onlineDate={data.seller.updatedAt}
...

              {
                ...
                >
                  <Comments
                    user={user}
                    page={page}
                    loadMoreComments={loadMoreComments_}
                    searchAgain={searchAgain}
                    all={false}
                    pass={pass}
                    isLoading={isLoading}
                    setIsLoading={setIsLoading}
                    setters={{ setOnNewComment: setOnNewComment }}
                    reshapedComments={reshapedComments}
                    onNewComment={onNewComment}
                    setOnNewComment={setOnNewComment}
                    navigation={navigation}
                    product={data}
                  />
                  {isLoading && (
            ...
            {isLoading ? (
              ...
              <>
                <View style={[productDetailsStyles.likeContainer]}>
                  <View style={{ height: 20 }}></View>
                  <Pressable
                    style={[productDetailsStyles.likeAdders]}
                    onPress={() => {
                      likeAdders.length > 0
                        ? navigation.navigate({
                            name: "Followers",
                  ...
                  >
                    <View style={[productDetailsStyles.likeTitle]}>
                      <Text
                        style={[customText.text, productDetailsStyles.someText]}
                      >
                        ...ont aimé
                      </Text>
                    </View>
                    ...
                    <View
                      style={[
                        productDetailsStyles.likeContent,
                        {
                          flexDirection:
                            likeAdders?.length <= 0 ? "column" : "row",
                        },
                      ]}
                    >
                      <FlatList
                        ...
                        renderItem={({ item }) => {
                          return (
                            <View style={[productDetailsStyles.likeItem]}>
                              <Image
                                source={{
                                  uri: `${usersImagesPath}/${item.image}`,
                                }}
                                style={[productDetailsStyles.likeAddersImages]}
                              />
                              <Text style={[customText.text]}>
                                {item.username.substring(0, 4)}...
                              </Text>
                            </View>
                          );
                        }}
                      ...
                      {likeAdders?.length > 0 && (
                        <Pressable>
                          ...
                          >
                            {likeAdders?.length} like
                            {likeAdders?.length > 1 ? "s" : false} {">>"}
                          </Text>
                        </Pressable>
                      )}
                    </View>
                ...
                <View style={[productDetailsStyles.similarContainer]}>
                  ...
                  <View style={[productDetailsStyles.similarProducts]}>
                    <HorizontalProductsList
                      ...
                      replace={true}
                      styles={{
                        productContainer: { backgroundColor: appColors.white },
                        flatlist: { backgroundColor: appColors.white },
                      }}
                    />
                  </View>
                </View>
      ...
      {showPriceDetails && (
        <View style={[productDetailsStyles]}>
          <PriceDetails
            products={[data]}
            closePriceDetails={setShowPriceDetails}
            title="Informations Sur La Vente"
          />
        </View>
      )}
        ...
        <View style={[productDetailsStyles.propositionPrice]}>
          <View>
            {hasPropositionPrice(data) ? (
              <Text
                style={[customText.text, productDetailsStyles.offerLeftText]}
              >
                Vous avez reçu une offre spéciale pour ce produit!
              </Text>
            ) : (
              <Text
                style={[customText.text, productDetailsStyles.offerLeftText]}
              >
                Ce produit a déja été vendu!
              </Text>
            )}
          </View>
          {/*
      ...
      <View style={[productDetailsStyles.bottom]}>
        <Pressable
          style={[productDetailsStyles.button, productDetailsStyles.price]}
          onPress={() => {
            ...
            >
              <Text
                ...
                style={[
                  customText.text,
                  productStyles.price,
                  productDetailsStyles.buttonText,
                  {
              ...
              >
                {formatMoney(choosePrice(data))} XAF
              </Text>
            </View>
          ) : (
            <Text
        ...
        <View style={[productDetailsStyles.panier]}>
          ...
          {user._id != data.seller._id && data.sold === 0 ? (
            <Pressable
              ...
              onPress={() => {
                navigation.navigate("Offers", {
                  product: data,
                  inputFocused: true,
                  notificationUsers: { seller: data.seller, buyer: user },
                  notificationsOffers:
            ...
            <Pressable
              style={[productDetailsStyles.button]}
              onPress={() => {
                navigation.navigate("Notifications", { key: 1 });
              }}
            >
              <Text
                numberOfLines={1}
        ...
        <View style={[productDetailsStyles.acheter]}>
          {user._id == data.seller._id || data.sold === 1 ? (
            <CustomButton
              text={data.sold === 1 ? "Vendu" : "Acheter"}
              disable={true}
              styles={{
                pressable: productDetailsStyles.button,
                text: productDetailsStyles.buttonText,
              }}
