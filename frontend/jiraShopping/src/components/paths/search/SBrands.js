  ...
  const BrandItem = (props) => {
    ...
    return (
      <Pressable style={[brandItemStyles.itemContainer]}>
        <CheckBox
          title={item.name}
          containerStyle={[brandItemStyles.contentContainer, {}]}
          textStyle={[customText.text, brandItemStyles.checkBoxText]}
          checked={
            Object.keys(selectedLocalBrand).length > 0
              ? selectedLocalBrand[item.name]
              : selectedBrandFromContext[item.name]
          }
          onPress={() => {
            handleUserChoice(item.name);
          }}
        />
      </Pressable>
    );
    ...
    <View style={[brandItemStyles.container, brandItemStyles.cardItem]}>
      <FlatList
        ...
        renderItem={({ item }) => {
          return (
            <BrandItem
              tag="brand"
              item={item}
              updateSelectedLocalBrands={updateSelectedLocalBrands}
              selectedLocalBrand={selectedLocalBrand}
            />
          );
        }}
        keyExtractor={(item) => {
        ...
        ItemSeparatorComponent={(item) => {
          return <View style={brandItemStyles.categorySeparator}></View>;
        }}
        horizontal={false}
        numColumns={1}
