...
const UserLogin = (props) => {
  ...
  const signUpUser = async (email, username, password) => {
    try {
      ...
      signupUserWithEmailAndPassword(email, username, password).then(
        async () => {
      ...
      if (error instanceof Yup.ValidationError) {
        ...
        Alert.alert(
          "Error",
          "Une erreur est sruvenue lors de la verificaiton de vos informations. Vérifiez les données fournies."
        );
      }
    } finally {
      setIsLoading(false);
  ...
  const loginUser = async (email, username, password) => {
    try {
      ...
      user = await loginUserWithEmailAndPassword(
        newUser.email,
        newUser.username,
        newUser.password
      );
    ...
    >
      <View style={[userLoginStyles.container]}>
        <ImageBackground
          ...
          style={userLoginStyles.backgroundImage}
        >
          ...
          <View style={[userLoginStyles.infoContainer]}>
            <View style={[userLoginStyles.registerOrLogin]}>
              <RadioButton.Group
                onValueChange={(newValue) => setCredentialType(newValue)}
            ...
            <View style={[userLoginStyles.credentialContainers]}>
              <Input
                ...
                inputContainerStyle={[
                  userLoginStyles.inputContainer,
                  isEmailFocused && userLoginStyles.inputFocused,
                ]}
                leftIcon={
                  <Pressable onPress={() => {}}>
                    <Icon
              ...
              />
              {errors.email && (
                <Text style={[formErrorStyle.text]}>{errors.email}</Text>
              )}
            </View>
            ...
            <View style={[userLoginStyles.credentialContainers]}>
              <Input
                ...
                inputContainerStyle={[
                  { borderBottomWidth: 1 },
                  isPasswordFocused && userLoginStyles.inputFocused,
                ]}
                leftIcon={
                  <Pressable onPress={() => {}}>
                    <Icon
              ...
              />
              {errors.password && (
                <Text style={[formErrorStyle.text]}>{errors.password}</Text>
              )}
            </View>

            <Pressable
              style={[userLoginStyles.forgotBox]}
              onPress={() => {
                navigation.navigate("ResetPassword");
              }}
            >
              <Text style={[customText.text, userLoginStyles.forgotText]}>
                Mot de passe oublié ?
              </Text>
            </Pressable>
            ...
            <CustomButton
              ...
              styles={{
                pressable: userLoginStyles.pressable,
                text: userLoginStyles.text,
              }}
            />
          </View>
          <Text>
            {/* <Button title="Sign Up" onPress={()=>{navigation.navigate("UserSignup")}} /> */}
          </Text>
        </ImageBackground>
        ...
        <CustomModalActivityIndicator
          ...
          size="large"
          color={appColors.secondaryColor1}
          message="Vérification des donnéees..."
        />
      </View>
    </KeyboardAwareScrollView>
  );
