...
const ProductsListWithFilters = React.forwardRef((props, ref) => {
    const { datas, horizontal, title, onEndReached, onEndReachedThreshold, onScroll, filters, notDisplayFilters, name, searchText, isLoading, 
        getDatas, display, onExit, setOnExit, minified, updateProfileLike, origin, search, previousScreen, category, emptyIcon
    } = props;
    ...
    return(
            <View style={[productsListWithFiltersStyles.container]}>
                ...
                <View style={{flex:1,flexDirection:"column",}}>
                    {filters &&
                        <Filters category={category} previousScreen={previousScreen} search={search}  onExit={onExit} setOnExit={setOnExit} getDatas={getDatas} notDisplayFilters={notDisplayFilters || {}} suggestion={false} searchText={searchText} display={display}/>
                    }
                        <ProductsList emptyIcon={emptyIcon} origin={origin} updateProfileLike={updateProfileLike}  minified={minified} name={name} onScroll={onScroll || function(){}} isLoading={isLoading} onEndReached={onEndReached||function(){}} onEndReachedThreshold={onEndReachedThreshold} ref={ref} datas={datas} horizontal={false} styles={preferencesStyles} />
                </View>
