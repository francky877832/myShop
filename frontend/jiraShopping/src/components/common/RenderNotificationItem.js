...
const RenderNotificationItem = (props) => {
  ...
  useEffect(() => {
    if (
      ["seconde", "secondes", "minute", "minutes"].includes(
        sinceDate(item.updatedAt)[1]
      )
    ) {
  ...
  return (
    <View style={[notificationsStyles.item]}>
      <Pressable
        ...
        style={[
          notificationsStyles.pressable,
          read ? notificationsStyles.itemRead : false,
          {},
        ]}
      >
        <View style={[notificationsStyles.icon, {}]}>{getIcon(from)}</View>

        <View style={[notificationsStyles.content, {}]}>
          <View style={[notificationsStyles.title, {}]}>
            <Text style={[customText.text, notificationsStyles.titleText]}>
              {item.title}
            </Text>
          </View>
          ...
          <View style={[notificationsStyles.message, { flexDirection: "row" }]}>
            ...
            <Text
              style={[customText.text, notificationsStyles.messageText, {}]}
            >
              {item.message}
            </Text>
          </View>
      ...
      <View style={[notificationsStyles.sinceDate, {}]}>
        <Text style={[customText.text, notificationsStyles.textDate, {}]}>
          il y'a {date}
        </Text>
      </View>
    </View>
