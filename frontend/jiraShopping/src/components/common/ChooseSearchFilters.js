...
const ChooseSearchFilters = (props) => {
  ...
  if (page == "category" || params?.datas?.page === "category") {
    ...
    const handleNavigationSubCat = (category) => {
      navigation.navigate("SSubCategories", {
    ...
    const CategoriesSearchItem = (props) => {
      ...
      const choices = Object.keys(selectedModalCategoriesFromContext).filter(
        (key) => {
          return selectedModalCategoriesFromContext[key] === true;
        }
      );
      const catAcc = choices.map((el) => {
        return { category: el.split("/")[0], subCategory: el.split("/")[1] };
      });
      ...
      return (
        <Pressable
          style={[filtersSearchStyles.pressableFilter]}
          onPress={() => {
            handleNavigationSubCat(item);
          }}
        >
          <View style={[{}]}>
            <Text style={[customText.text, filtersSearchStyles.itemText]}>
              {item.name}
            </Text>
          </View>
          ...
          <View style={[{ justifyContent: "center" }]}>
            ...
            {selectedFilters[item.name] && (
              <Text
                style={[customText.text, filtersSearchStyles.selectedFilters]}
              >
                {splitSelectedFilters(selectedFilters[item.name]?.join(", "))}
              </Text>
            )}
          </View>
        </Pressable>
      ...
      <View style={[sCategoriesStyles.container]}>
        ...
        <View style={[sCategoriesStyles.categoriesContainer]}>
          <FlatList
            ...
            renderItem={({ item }) => {
              return <CategoriesSearchItem item={item} />;
            }}
            keyExtractor={(item) => {
