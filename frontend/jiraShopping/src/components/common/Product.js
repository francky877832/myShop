...
import { LikeButton, CommentSection } from "./CommonSimpleComponents";
...
import {
  scheduleNotificationAfterAction,
  cancelNotification,
  cancelAndSendNotificationAfterAction,
...
const Product = (props) => {
  ...
  const [product, setProduct] = useState(
    modifiedProduct.length > 0 ? modifiedProduct[0] : { ...item }
  );
  ...
  const [isFavourite, setIsFavourite] = useState(
    useSelector((state) => isProductFavourite(state, product._id), shallowEqual)
  );
  const [isBasketPresent, setIsBasketPresent] = useState(
    useSelector((state) => isProductBasket(state, product._id), shallowEqual)
  );
  ...
  const handleBasketPressed = (product) => {
    if (isBasketPresent) {
      ...
      timeoutRef.current = setTimeout(async () => {
        ...
        const notificationId = await cancelAndSendNotificationAfterAction(
          notifications,
          "basket",
          "ON_BASKET_ADD",
          3600 * 12
        ...
        if (notificationId) {
          dispatch(
            updateNotifications({ name: "ON_BASKET_ADD", id: notificationId })
          );
        }
      }, 1000);
    }
...
{*/
  ...
  const printTopIcon = (product) => {
    let icon = "";
    switch (true) {
      case product.newPrice < product.price:
        icon = "reduction";
        break;
      case product.feesBy === "seller":
        icon = "gratuit";
        break;
      default:
        icon = "proposition";
    ...
    return (
      <Pressable
        style={[/*productStyles.card,*/ productStyles.feesBy]}
        onPress={() => {}}
      >
        <Icon
          name={topIcons[icon].name}
          type={topIcons[icon].type}
          size={18}
        ...
        <Text
          style={[customText.text, { color: appColors.white, fontSize: 12 }]}
        >
  ...
  const printBottomIcon = (product) => {
    ...
    return icon.map((item, key) => {
      return (
        <Pressable
          style={[productStyles.bottomIconsButton]}
          onPress={() => {}}
          key={key}
        >
          <Icon
            name={topIcons[item].name}
            type={topIcons[item].type}
            size={22}
            color={topIcons[item].color}
          />
  ...
  function displayGrille(origin, product) {
    return (
      origin === "profileShop" && (
        <View style={[productStyles.containerVisibility]}></View>
      )
    );
  ...
  function displayGrilleBttom(origin, product) {
    return (
      origin === "profileShop" && (
        <Pressable
          style={[
            productStyles.containerVisibilityInfo,
            {
              backgroundColor:
                product.sold == 1 ? appColors.green : appColors.secondaryColor5,
            },
          ]}
          onPress={() => {
            product.sold == 1 ? showConfirmationRenewProduct(product) : null;
          }}
        >
          <Text
            style={[
              { color: appColors.white, fontSize: 12, fontWeight: "bold" },
            ]}
          >
            {product.sold == 1
  ...
  return (
    <View
      style={[
        ...
        productStyles.card,
        horizontal ? productStyles.containerHorizontal : false,
      ]}
    >
      {product.visibility == 0 && displayGrille(origin, product)}

      {(product.visibility == 0 || product.sold == 1) &&
        displayGrilleBttom(origin, product)}
      <View
        style={[
          productStyles.top /*{justifyContent:(product.feesBy=="seller" || product.newPrice < product.price)?'space-between':'flex-end'}*/,
        ]}
      >
        {!bottomIcon && (
      ...
      <Pressable style={productStyles.pressable} onPress={handlePress}>
        <Image
          source={{
            uri: "https://th.bing.com/th/id/OIF.prI1qbZoZzd4gaK8SnlWEw?rs=1&pid=ImgDetMain",
          }}
          // source={{ uri: `${productsImagesPath}/${product.images[0]}` }}
          style={[
            productStyles.image,
            horizontal ? productStyles.imageHorizontal : false,
          ]}
        />
        <View style={productStyles.text}>
          ...
          >
            ...
            <Text
              numberOfLines={3}
              style={[, customText.text, productStyles.productName]}
            >
              {product.name}
            </Text>
            <Text style={{ width: "35%", flexDirection: "row" }}>
              {(!minified || user._id != product.seller._id) && (
                <LikeButton
                  ...
                  item={product}
                  isCard={false}
                  styles={{ color: appColors.black }}
                />
              )}
              <CommentSection style={{ width: "50%" }} />
            </Text>
          </View>
          <View style={[{ flexWrap: "wrap" }]}>
            ...
            {product.price <= product.newPrice ? (
              <Text
                style={[customText.text, productStyles.price, { fontSize: 11 }]}
              >
          ...
          >
            <View style={{ width: "70%" }}>
              {!minified || user._id != product.seller._id ? (
                <View
                  style={[
                    productStyles.bottom,
                    // productStyles.card,
                    isBasketPresent ? productStyles.isBasketPresent : false,
                  ]}
                >
                  <Pressable
                    style={{
                    ...
                    onPress={() => {
                      handleBasketPressed(product);
                    }}
                  >
                    <Ionicons
                      name={isBasketPresent ? "checkmark" : "add"}
                      size={15}
                      color={
                        isBasketPresent
                    ...
                    <Text
                      ...
                      style={[
                        ...
                        ,
                        isBasketPresent
                          ? productStyles.isBasketPresentText
                          : false,
                      ]}
                    >
                      {isBasketPresent
                ...
                <View style={[productStyles.bottom, productStyles.card, {}]}>
                  <Pressable
                    onPress={() => {
                      handleMofyProduct(product);
                    }}
                  >
                    <Text
                      numberOfLines={1}
                      style={[customText.text, productStyles.category]}
                    >
                      Modifier le produit
                    </Text>
                  </Pressable>
