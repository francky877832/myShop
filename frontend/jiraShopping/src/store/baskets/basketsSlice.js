  ...
  async ({ product, user }, { rejectWithValue }) => {
    ...
    try {
      const response = await fetch(
        `${server}/api/datas/basket/update/${user._id}`,
        {
          method: "POST",
...
export const removeFromBasket = createAsyncThunk(
  ...
  async ({ product, user }, { rejectWithValue }) => {
    ...
    try {
      const response = await fetch(
        `${server}/api/datas/basket/remove/${user._id}`,
        {
          method: "PUT",
          body: JSON.stringify(basket),
          headers: { "Content-Type": "application/json" },
...
const basketSlice = createSlice({
  ...
  reducers: {
    ...
    updateSelectedProducts: (state, action) => {
      ...
      Object.keys(updatedSelectedProducts).forEach((key) => {
        ...
        if (isSelected && item) {
          total +=
            choosePrice(item) *
            ((state.selectedProducts[item._id] && state.quantities[item._id]) ||
              1);
        } else {
          total += 0;
        }
    ...
    removeLocalBasket(state, action) {
      ...
      const existingIndex = state.basket.findIndex(
        (basketItem) => basketItem._id === itemId
      );
