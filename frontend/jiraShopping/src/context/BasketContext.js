...
const BasketProvider = ({ children }) => {
  ...
  const addBasket = async (item) => {
    ...
    try {
      response = await fetch(
        `${server}/api/datas/basket/update/${loggedUserId}`,
        {
          method: "POST",
  ...
  const removeBasket = async (item) => {
    ...
    try {
      response = await fetch(
        `${server}/api/datas/basket/remove/${loggedUserId}`,
        {
          method: "PUT",
          body: JSON.stringify(basket),
          headers: {
...
*/
  ...
  const fetchUserBasket = async () => {
    try {
      ...
      const response = await fetch(
        `${server}/api/datas/basket/get/${loggedUserId}`
      );
  ...
  return (
    <BasketContext.Provider
      value={{ ...basketStateVars, ...basketStateStters, ...utilsFunctions }}
    >
      {children}
    </BasketContext.Provider>
  );
