...
const CommentsProvider = ({ children }) => {
  ...
  const fetchProductComments = async (id, page) => {
    ...
    try {
      ...
      const response = await fetch(
        `${server}/api/datas/comments/get/${id}?page=${page}`
      );
  ...
  const fetchProductLastComment = async (id, userId) => {
    ...
    try {
      //console.log("Ok")
      const response = await fetch(
        `${server}/api/datas/comments/get/last/${id}?user=${userId}`
      );
    ...
    async (product) => {
      ...
      if (onNewComment) {
        try {
          ...
          console.error(
            "Erreur lors du chargement des commentaires onNewComment :",
            error
          );
        } finally {
          setIsLoading(false);
          ...
          updatedComments = prevComments.map((cm) => {
            if (cm._id == comment_.isResponseTo) {
              ...
              return {
                ...cm,
                subComment: [
                  ...subComment.slice(0, subComment.length - 1),
                  comment_,
                ],
              };
            }
