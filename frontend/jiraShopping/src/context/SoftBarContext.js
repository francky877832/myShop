...
const SoftBarProvider = ({children}) => {
  ...
  // Calculez la hauteur de la barre de navigation virtuelle
   const BOTTOM_NAV_BAR_HEIGHT = WINDOW_HEIGHT - (Dimensions.get('screen').height - insets.bottom - (StatusBar.currentHeight || 0));

    const filterStateVars = {BOTTOM_NAV_BAR_HEIGHT}
    const filterStateSetters = {}
    const utilsFunctions = {  }
    return (
        <SoftBarContext.Provider value={{...filterStateVars, ...filterStateSetters, ...utilsFunctions}}>
            {children}
