...
const FavouritesProvider = ({ children }) => {
    ...
    try {
      if (bool) {
        response = await fetch(
          `${server}/api/datas/favourites/update/${loggedUserId}`,
          {
            method: "POST",
        ...
        //console.log("fine")
        response = await fetch(
          `${server}/api/datas/favourites/remove/${loggedUserId}`,
          {
            method: "PUT",
            body: JSON.stringify(favourite),
            headers: {
  ...
  const fetchUserFavourites = async (username, page) => {
    try {
      ...
      const response = await fetch(
        `${server}/api/datas/favourites/get/${username}?page=${page}`
      );
  ...
  const isFavouritePresent = (item) => {
    ...
    const index = bool
      ? favourites.findIndex((fav) => fav._id === item._id)
      : -1;
