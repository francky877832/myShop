...
const FilterProvider = ({ children }) => {
  ...
  const [selectedConditionsFromContext, setSelectedConditionsFromContext] =
    useState({});
  ...
  const [allCategoriesSelected, setAllCategoriesSelected] = useState(
    Object.keys(selectedModalCategoriesFromContext).some(
      (el) => selectedModalCategoriesFromContext[el] === false
    )
  );
  ...
  const getSearchedTextWithFilters = useCallback(
    async ({
      searchText,
      orderBy,
      ...
      let categories;
      if (
        Object.keys(selectedModalCategories).length > 0 ||
        searchText.trim().length > 0
      ) {
        categories = Object.keys(selectedModalCategories).filter((key) => {
          return selectedModalCategories[key] === true;
        });
        ...
        if (Object.keys(selectedCategory).length <= 0) {
          ...
          categories = Object.keys(selectedCategory).filter((key) => {
            return selectedCategory[key] === true;
          });
          if (!!selectedCategory.subCategories) {
      ...
      //GESTION DES MARQUES
      let brands = Object.keys(selectedBrands).filter((key) => {
        return selectedBrands[key] == true;
      });

      //COLORS
      let colors = Object.keys(selectedColors).filter((key) => {
        return selectedColors[key] == true;
      });
      ...
      try {
        const response = await fetch(
          `${server}/api/datas/products/search?${queryString}`,
          {
            method: "GET",
            headers: {
        ...
        if (!response.ok)
          throw new Error(
            `Erreur lors de la recherhce de produit${response.text()}`
          );
  ...
  const loadMoreDataWithFilters = useCallback(
    async ({
      searchText,
      orderBy,
      selectedModalCategories,
      ...
      try {
        ...
        const newData = await getSearchedTextWithFilters({
          searchText: searchText,
  ...
  const filterStateVars = {
    ...
    selectedCategories,
    selectedOrderBy,
    selectedBrandFromContext,
    selectedModalCategoriesFromContext,
    selectedConditionsFromContext,
    isNewFocused,
    isOldFocused,
    minPrice,
    maxPrice,
